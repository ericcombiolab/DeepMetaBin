...
def csr2dict(X_csr, keys):
    ...
    for (r, c, d) in zip(Xcoo.row, Xcoo.col, Xcoo.data):
        ...
        if map_dict_reverse[r] != map_dict_reverse[c]:
            x_convert[map_dict_reverse[r]].append(map_dict_reverse[c])
...
# -------------------------
tnfs = np.load('../data/sharon/vamb/tnf.npz')
abundances = np.load('../data/sharon/vamb/rpkm.npz')
tnfs = tnfs['arr_0']
...
train_set = []
test_set = []
contigs = np.load('../data/sharon/vamb/contignames.npz')
contigs = contigs['arr_0']
graph = torch.load('../data/sharon/ag_dict.pkl')

# sqaure/cube the assembly graph
X_csr = dict2csr(graph)
# X_csr = csr_matrix_mul(X_csr)
# X_csr = csr_matrix_mul(X_csr)
X_csr = X_csr.multiply(X_csr)
X_csr = X_csr.multiply(X_csr)
graph = csr2dict(X_csr,graph.keys())
...
torch.save(train_set, '../data/sharon/training_set.pkl')
torch.save(test_set, '../data/sharon/test_set.pkl')
